...
const TopBooks = () => {
  ...
  const [reviews, setReviews] = useState<ReviewsState>(
    [] as unknown as ReviewsState
  );
  const [isbn, setIsbn] = useState<number | undefined>(undefined);
  const params = useParams();

  const topbooksQuery = useQuery(
    `topbooks/${params.listName}`,
    () => {
      if (!params.listName) {
        return;
      }
      return NYTimesClient.bestsellersListNameEncodedGet(params.listName).then(
        (response) => {
          ...
          <TableBody>
            {books.map((book) => (
              <React.Fragment key={book.isbn}>
                ...
                {book.isbn && reviews[parseInt(book.isbn)] && (
                  <Reviews reviews={reviews[parseInt(book.isbn)]} />
                )}
              </React.Fragment>
            ))}
          </TableBody>
